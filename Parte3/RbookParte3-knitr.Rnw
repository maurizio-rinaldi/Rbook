...
library(knitr)
opts_knit$set(progress = TRUE, verbose = TRUE,warning=F,message=F)
options(formatR.arrow=TRUE,width=50,warning=F)
opts_chunk$set(fig.path='figure/graphics-', cache.path='cache/graphics-', fig.align='center', dev='tikz', fig.width=5, fig.height=4, fig.show='hold', cache=TRUE, par=TRUE,warning=FALSE,message=FALSE)
...
Consideriamo una\subsection{Indicatori statistici}
\begin{itemize}
\item{}Media.\vskip0pt
...
\(P(\textrm{Rosso}\mid \textrm{Pari}) \)= \Sexpr{  length(intersect(pari,rosse))}/\Sexpr{ length(pari)}
...
\(P(\textrm{Rosso}\cap \textrm{Pari})\)= \Sexpr{  length(intersect(pari,rosse))}/\Sexpr{ length(temp)}
...
<<echo=FALSE>>=
plot(0,type="n",xlab="",ylab="",xlim=c(0,1),axes=F)
segments(0,0,1,0,lwd=1)
points(0.5,0,col="red",pch=2)
text(0.5,0.1,expression(mu),cex=2)
 
segments(0,-0.5,1,-0.5,lwd=1)
segments(0.4,-0.5,0.6,-0.5,lwd=4,col="blue")
text(0.5,-.4,expression(mu),cex=2)
text(0.4,-.5,"(",cex=2)
text(0.6,-.5,")",cex=2)
@
...
Scegliamo un livello di fiducia, per esempio il 95%. Sappiamo alllora che esiste un numero $u$ tale che l'area indicata valga 0.95.  Possiamo essere allora certi (nel senso appena spiegato) che la variabile standardizzata cada nell'intervallo $[-u,u]$.
...
($1-\epsilon$).
<<echo=FALSE>>= 
shadeareastandard<-function(a,b) 
        {sigma=1;mu=0
        curve(dnorm(x),lwd=4,xlab="",xlim=c(-3*sigma+mu,max(b,3*sigma+mu)),ylim=c(0,1.2*dnorm(0,0,sigma)),axes=F,xaxt="n",ylab="")
        axis(1,at=c(a,b),labels=c("-u","u"),)
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
text(a/2 ,dnorm(a/2+b/2)/4,bquote("P(-u<X<0)=0.475")  ,cex=1)
text(b/2 ,dnorm(a/2+b/2)/4,bquote("P(0<X<u)=0.475")  ,cex=1)
abline(v=0)
     }
shadeareastandard(-1.96,1.96)
@
...
tabnormale<-cbind(
matrix(seq(start,stop,by=step),ncol=nc),matrix(correzione(seq(start,stop,by=step)),ncol=nc))
as.vector(t(matrix(c(nc+1:nc,1:nc),nc=2)))->ordinecol
colnames( tabnormale)= rep(c( "x","Area"),each=nc) 
normale=tabnormale[,c(1,4,2,5,3,6)] 
pander(normale)
@
Guardando la tabella la soluzione non è  immediata. Se per esempio scegliamo un livello di fiducia del 95%, dobbiamo valutare per quale valore di $u$ l'area da 0 ad  $u$ è  $0.95/2=0.475$ (perché anche l'area da $-u$ a 0 sarà  uguale a 0.475). Dalla tabella si evince che tale valore è  compreso tra 1.9 e 2.0.  
...
nc=3;
cifre=4;
proba=c(seq(start,fine-step,by=step),0.999,fine)
correzione<-function(x)  format(round(10^cifre* (qnorm((1+x)/2)))/10^cifre,nsmall=4)
normaleinversa<-cbind(
matrix(format(proba,nsmall=3),ncol=nc),matrix(correzione(proba),ncol=nc))
as.vector(t(matrix(c(nc+1:nc,1:nc),nc=2)))->ordinecol
colnames( normaleinversa)= rep(c( "1-epsilon","u"),each=nc)
#rownames( tabnormale)=rep("",nrow( tabnormale))
normaleinversa=normaleinversa[,c(1,4,2,5,3,6)] 
 
#normale

pander(normaleinversa)
shadeareastandar<-function(a,b) 
        {sigma=1;mu=0
        curve(dnorm(x),lwd=4,xlab="",ylab="",xlim=c(-3*sigma+mu,max(b,3*sigma+mu)),ylim=c(0,1.2*dnorm(0,0,sigma)),axes=F,xaxt="n")
        axis(1,at=c(a,b),labels=c("-u","u"),)
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
text(0 ,dnorm(a/2)*0.7,expression(paste("1-",epsilon,sep=""))  ,cex=1)
 
     }
shadeareastandar(-1.96,1.96)
@
...
shadeareastandard<-function(a,b) 
        {sigma=1;mu=0
        curve(dnorm(x,mu,sigma),lwd=4,xlab="",ylab="",xlim=c(-3*sigma+mu,max(b,3*sigma+mu)),ylim=c(0,1.2*dnorm(0,0,sigma)),axes=F,xaxt="n")
        axis(1,at=c(a,b),labels=c("-u","u"),)
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
text(-0,0.15,expression(paste("A"[1],"=pnorm(u)"))  ,cex=1)
}
shadeareastandard(-4,1.96)
arrows(2,-0.015,0,0.12,lwd=2,col="tan1")
@
...
shadeareastandard<-function(a,b) 
        ...
        axis(1,at=c(a,b),labels=c("-u",expression(paste("u=qnorm(A"[1],")"))))
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
text(-0,0.15,expression("A"[1])  ,cex=1)
}
shadeareastandard(-4,1.96)
arrows(0,0.12,2,-0.015,lwd=2,col="tan4")
@
...
s2<-function(a,b) 
        {sigma=1;mu=0
        curve(dnorm(x),lwd=4,xlab="",xlim=c(-3*sigma+mu,max(b,3*sigma+mu)),ylim=c(0,1.2*dnorm(0,0,sigma)),axes=F,xaxt="n",ylab="")
        axis(1,at=c(a,b),labels=c("-u",expression(paste("u=U(A"[2],")=U(1-",epsilon,")"))))
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
text(-0,0.15,expression(paste("A"[2],"=1-",epsilon))  ,cex=1)
}
s2(-1.96,1.96)
@
...
s3<-function(a,b) 
        ...
        axis(1,at=c(a,b),labels=c("-u",expression(paste("u=qnorm(A"[1],"/2+1/2)"))))
        x1 <- 0  
        x2 <- b
        x=seq(x1,x2,by=0.01)
        polygon( c(x1,x,x2) , c(0, dnorm(x), 0), col="light green")
...
text(1,0.15,expression("A"[2]/2)  ,cex=1)
text(0,0.43,expression(paste("A"[1],"=1/2+A"[2]/2))  ,cex=2)
}

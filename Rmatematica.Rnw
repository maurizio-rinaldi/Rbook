...
for (i in 1:10){
<<chunk1>>
testo=paste(testo,paste("\\\\item ",fractions(q/p),"+",
fractions(s/r)),sep="\n")
soluzioni=paste(soluzioni,paste("\\\\item ",fractions(q/p)+
fractions(s/r)),sep="\n")
}
frazionisol=soluzioni
@
...
\begin{center}
<<echo=FALSE,fig=TRUE>>=
curve(1/x,-2,2,xlim=c( -3,3), ylim=c(-4,4)) 
@
%\includegraphics[scale=0.4]{grafici/Rplot34.pdf}
...
<<echo=FALSE, fig=TRUE>>=
par(mfrow=c(2,2))
plot(0,0,type='n',axes=FALSE,ann=FALSE,asp=1)
mtext("x",side=4,las=2)
mtext("y",side=3)
abline(v=0)
abline(h=0)
plot(0,0,type='n',axes=FALSE,ann=FALSE ,asp=1)
mtext("x",side=2,las=2)
mtext("y",side=3)
abline(v=0)
abline(h=0)
plot(0,0,type='n',axes=FALSE,ann=FALSE)
mtext("y",side=4,las=2)
mtext("x",side=3)
abline(v=0)
abline(h=0)
plot(0,0,type='n',axes=FALSE,ann=FALSE,asp=1)
...
title(main="Funzione invertibile",  font.main=4)
@
...
par(mfrow=c(2,2))
x=seq(-5,5,0.1)
plot(x,exp(x),type="l");abline(h=0,col="red");abline(v=0,col="red")
plot(-x,exp(x),type="l");abline(h=0,col="red");abline(v=0,col="red")
plot(exp(x),x,type="l");abline(h=0,col="red");abline(v=0,col="red")
 par(mfrow=c(1,1))
@
\end{figure}
...
<<fig=TRUE,echo=FALSE>>=
par(mfrow=c(2,1))#genera un pannello di 2 grafici 
par(mai=c(0.3,1,0.4,0.5))
par(mgp = c(1.7,0.5, 0))
x=seq(-pi/2,pi/2,0.01)
plot(x,sin(x),type="l")
plot(sin(x),x,type="l")
...
for (i in 1:10){
<<logeq>>
testo=paste(testo,paste("\\\\item ","$\\\\log_",a,"(",b, "x","+", k,")=",c,"$"))
soluzioni=paste(soluzioni,paste("\\\\item ",x,sep="\n"))
logeqsol=soluzioni}
@
...
testo=expression("x"["*"])
text( 1.6,1,testo)
points(x0,f(x0),cex=1,pch=19)
m=df(x0)
q=f(x0)-m*x0
x1=x0-f(x0)/m
curve(m*x+q,add=T,from=x1,to=x0)
segments(x0,0,x0,f(x0),col="brown")
testo=expression("x"[0])
text( x0,-1,testo)
testo1=expression("x"[1])
text( x1,-1,testo1)
@
\end{center}
...
q=f(x1)-m*x1
x2=x1-f(x1)/m
curve(m*x+q,add=T,from=x2,to=x1,col="red")
testo2=expression("x"["2"])
text( x2,-2,testo2) 
segments(x1,0,x1,f(x1),col="brown")
<<step0>>
testo1=expression("x"[1])
text( x1,-1,testo1)
m=df(x1)
...
q=f(x2)-m*x2
x3=x2-f(x2)/m
curve(m*x+q,col="red",from=x2,to=x3,add=T)
testo3=expression("x"["3"])
text( x3,-2,testo3) 
segments(x2,0,x2,f(x2),col="brown")
@

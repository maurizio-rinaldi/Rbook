...
\Sexpr{paste(R.Version()$major,R.Version()$minor)}.
...
@  
\index{\texttt{setwd()}} 
 imposta la {\it working directory}  nella posizione specificata da \texttt{"indirizzo"}. Lavorando con \textsf{RStudio} per impostare la directory si pu\`o usare il menu a tendina
\texttt{Session} e in particolare  posizionare la working directory in corrispondenza al file di codice sorgente.  \newpage
\thispagestyle{empty}


\subsection*{Pacchetti  }
\subsection{Pacchetti}
Oltre all'aggiornamento costante del programma, lo sviluppo di \textsf{R} consiste nello creazione e aggiornamento continui da parte della comunit\`a  mondiale di una serie di pacchetti addizionali. Un pacchetto contiene in particolare un insieme di comandi finalizzati alla  realizzazione di un determinato compito: esistono per esempio pacchetti per la risoluzione di equazioni differenziali, pacchetti per l'analisi statistica di {\it microarray}, pacchetti per il {\it clustering} di dati.  Per non appesantire \textsf{R} i pacchetti (con poche eccezioni) vengono caricati solo se richiesto dalle esigenze dell'utente. 
L'utente deve quindi inizialmente localizzare (per esempio via web) il pacchetto che lo interessa e scaricarlo localmente.  
Per fare questo deve selezionare il sito CRAN da cui scaricarlo e poi (direttamente dal menu a tendina di \textsf{R}) scaricarlo (con annesse eventuali {\it dependencies}). Una volta scaricato in locale il pacchetto pu\`o essere messo a disposizione dell'utente con il comando  
%codechunk
<<echo=TRUE,eval=FALSE>>=
...
\begin{itemize}
\item Riconoscere la sintassi di R 
\item Interagire con R, consentendo l'esecuzione di comandi direttamente dall'editor. 
\end{itemize}
...
%Per affiancare verticalmente due o pi\`u finestre in ambiente {\textsf Windows} si seleziona l'opzione ${\it finestre} \rightarrow {\it affianca}$ dal menu a tendina
%oppure per installazione in lingua inglese:  ${\it windows} \rightarrow{\it  tile}$. 
Adotteremo in questo libro RStudio per la facilit\`a d'uso e per il fatto che consente una visualizzazione completa contestuale di diversi aspetti di una sessione di lavoro.
Se si lavora all'interno di RStudio il caricamento dei pacchetti \`e pi\`u semplice: in uno dei pannelli \`e presente il menu \texttt{Packages} dal quale l'utente pu\`o scaricare il pacchetto usando il sottomenu \texttt{Install} e caricarlo all'interno di R semplicemente barrandone la casellina relativa.

%\begin{comment}
\section{Rstudio}
All'apertura di \textsf{RStudio} appaiono diversi pannelli.
 
  \begin{figure} 
\includegraphics[width=0.8\textwidth]{grafici/menubar.png}
\caption{RStudio menubar }
 \end{figure}
 
\begin{figure} 
\includegraphics[height=80mm,width=160mm]{grafici/RStudio}
\caption{RStudio Screenshot}
\end{figure}

 \begin{figure} 
\includegraphics[width=0.8\textwidth]{grafici/Rstudioambiente.png}

\includegraphics[width=0.8\textwidth]{grafici/Rstudiofiles.png}
\includegraphics[width=0.8\textwidth]{grafici/Rstudiofiles.png}\caption{RStudio menubar }
...
Il pannello  fondamentale che appare aprendo \textsf{RStudio} è la classica console di R.
Ci sono poi 2 altre finestre (bipartite) una con 2 linguette  \texttt{Environment} e \texttt{History} la seconda con 5 linguette mostrate nella figura che segue.


\subsection{Script di codice in R e RStudio}
Ad esempio deve essere possibile inviare ad R  porzioni di codice per la loro esecuzione 
R stesso include un editor (basta selezionare New document nel menu a tendina presente sotto File, in Rstudio invece sotto FIle basta selezionare NewScript.

\end{document}
\section{Numeri, operazioni aritmetiche e variabili}
\subsection{I numeri}
Se scriviamo in \textsf{R} qualunque numero la rappresentazione che ne abbiamo \`e quella di un numero reale dove  il punto viene usato come separatore decimale. Inoltre per numeri molto grandi o molto piccoli \textsf{R} ricorre alla notazione scientifica. Un numero  ``grande''  viene scritto come
...
class(5)
@
ci accorgiamo che il numero intero 5 \`e stato interpretato come un numero reale.
Se desideriamo espressamente ricordarci che  \`e  un  intero possiamo scrivere
<<>>=
class(5L)
@
...
Possiamo inoltre lavorare con le frazioni caricando il pacchetto \texttt{MASS}
<<>>=
library(MASS)
...
\Sexpr{fractions(q/p)}+\Sexpr{fractions(s/r)}
\end{question}
\\
\begin{solution}
 
Si determina il denominatore comune
prendendo il minimo comune multiplo di \Sexpr{p} e \Sexpr{r}.
 \end{solution}
\end{itemize}
%% META-INFORMATION
%% \extype{string}
%% \exsolution{\Sexpr{fractions(q/p+s/r)}}
%% \exname{meaniris}
%% \extol{0.01}
...
I nomi delle variabili sottostanno ad alcune regole. Per esempio \texttt{variabile1} \`e un moine lecito mentre non lo \`e  \texttt{1variabile}.\\  L'assegnamento di uno stesso valore a pi\`u variabili (o costanti) in un unico comando \`e possibile con una sintassi del tipo: 
<<echo=TRUE>>=
a<-b<-c<-2 
@
...
objects()
@
L'insieme degli oggetti presenti costituisce l'\textsl{environment} (ambiente di lavoro). 
Si noti  che il numero di cifre di cui \textsf{R} dispone  è  superiore al numero di cifre visualizzate
<<>>=
a=4/3
a
a*3
...
pu\`o essere visto come un casellario, le cui caselle sono dette celle (\emph{slots}) del vettore stesso. Tutte le celle sono variabili di uno stesso tipo, detto tipo base del vettore. Si possono cos\`\i\  definire vettori di numeri, di caratteri e vettori logici. 
...
f<-function(x) 1/x
curve(f,0,2)
g<-function(x) 20*x;curve (g,add=T) 
@
...
options(width=55)
<<echo=TRUE>>
curve(1/x,xlim=c( -3,3), ylim=c(-4,4),col="dark blue")
@
...
par(mfrow=c(2,2))
d=d/2;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2"); abline(h=0);abline(v=0)
d=d/2;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^2"); abline(h=0);abline(v=0)
d=d/2;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf:  2^3"); abline(h=0);abline(v=0)
d=d/2;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^4"); abline(h=0);abline(v=0)
@
...
main="xf: 2^4"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^5"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^6"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf:  2^7"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^8"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d),
main="xf: 2^9"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d), 
main="xf: 2^10"); abline(h=0);abline(v=0)
d/2->d;curve(f,xlim=c(-d,d),ylim=c(-d,d), 
main="xf: 2^11"); abline(h=0);abline(v=0)
...
trepunti(f,x,h)
@
Otteniamo cos\`\i\; la stima della derivata nel punto \Sexpr{x} con incremento \Sexpr{h}.
...
curve(f,add=T,col="red")
curve(trepunti(f,x,0.001)/f(x),add=T, col="dark green")
@\caption{Grafico di $e^x$ e derivata}
\label{fig:grafexp}
\end{center}
\end{figure}
\end{itemize}
...
<<echo=FALSE,fig=TRUE>>=
f=function(x) x^3-5
curve (f,1,4,col="blue",xaxt="n",yaxt="n");abline(h=0)
testo=expression("x"["*"])
text( 1.8,-2,testo)
@
...
testo=expression("x"["*"])
text( 1.8,-2,testo)
testo=expression("x"[0])
text( 4,-2,testo)
segments(3.5,0,3.5,f(3.5),col="brown")
@
...
write.table(meteo,file="meteo.txt",sep=";")
<<echo=FALSE,eval=TRUE>>=
...
hist(Milano,col="red",freq=FALSE,breaks=c(12.5,15,17,18,20,22.5),
...
%code chunk
<<echo=TRUE>>=
studenti<-read.csv("http://www.pharm.unipmn.it/rinaldi/file_corso/studenti.csv")
head(studenti)
...
indirizzo=paste(sito,nomefile,sep="")
read.table(indirizzo)->crabs
dim(crabs)
<<echo=FALSE,eval=TRUE>>=
read.table("filedati/australian-crabs.txt",header=T)->crabs
dim(crabs)
@
...
<<echo=FALSE,eval=TRUE>>=
farmacia=read.table("filedati/datifarmacia.txt")
@  

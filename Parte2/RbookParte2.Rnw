...
s2<-function(a,b) 
        ...
        axis(1,at=c(a,b),labels=c("-u",expression(paste("u=U(A"[2],")"))))
        x1 <- a  
        x2 <- b
        x=seq(x1,x2,by=0.01)
...
text(1,0.15,expression("A"/2)  ,cex=1)
text(0,0.43,expression(paste("A"[1],"=1/2+A"/2))  ,cex=2)
}
 
@
Per definizione  $$\texttt{pnorm}(x)=\int_{-\infty}^x \texttt{dnorm}(s)ds=$$ mentre la funzione \texttt{qnorm} rappresenta la funzione inversa di \texttt{pnorm}.
Quindi
<<>>=
s4<-function(a,b) 
        {
...
text(1,0.15,expression("a")  ,cex=1)
text(-1,0.15,expression("1/2")  ,cex=1) 
}
par(mfrow=c(1,1))
s4(-2,2)
@
